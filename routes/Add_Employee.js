...
router.get("/", (req, res) => {
    res.render("Add_Employee", {pageTitle: "emp"});
});
    ...
    db_pool.query(q, [firstName, lastName], function (err, rows, fields) {
        if (err) {
            ...
            if (rows.length > 0) {
                ...
                let q = `INSERT INTO \`employees\` (\`FirstName\`,\`LastName\`,\`ImageURL\`,\`IDNumber\`,\`Address\`,\`Email\`,\`PhoneNumber\`,\`JobTitle\`,\`StartDate\`,\`HourlySalary\`,\`BankAccountDetails\`)
                VALUES ('${firstName}','${lastName}','${imageUrl}'),'${id}'),'${address}'),'${email}'),'${phone}'),'${jobTitle}'),'${startDate}'),'${hourlyRate}'),'${bankDetails}')`;
                console.log(q);
